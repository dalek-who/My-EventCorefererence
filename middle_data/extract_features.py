...
for k, topic_list in topics_train.items():
    for cross_document, cross_topic in [(False, False), (True, False), (True, True)]:
        positive_increase = 10 if cross_topic else 0
        csv_cross_doc = "_cdoc" if cross_document else "_wdoc"
        csv_cross_topic = "_ctpc" if cross_topic else "_wtpc"
        csv_pos_increase = "_pos-%s" % positive_increase if positive_increase else ""
        BERT_csv_dir = "./BERT_SentenceMatching_input/%s%s%s%s/" % (k, csv_cross_doc, csv_cross_topic, csv_pos_increase)
