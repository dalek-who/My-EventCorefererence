...
neleval prepare-tac -q $goldx $gtab $@ \
    > $gold
...
ls $sysdir/*.tab \
    | xargs -I{} -n 1 -P $jobs bash $SCR/run_tac14_prepare.sh $outdir {} $@
...
ls $outdir/*.combined.tsv \
    ...
    | xargs -n 1 -P $jobs bash $SCR/run_evaluate.sh $gold


# PREPARE SUMMARY REPORT
echo "INFO Preparing summary report.."
bash $SCR/run_tac14_report.sh $outdir